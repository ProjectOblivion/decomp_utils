...
def parse_psp_ovl_load(ovl_name, path_prefix, asm_path):
    ...
    ovl_load_name, ovl_load_symbol, ovl_header_symbol, entity_updates_name = None, None, None, None
    for file in (
        dirpath / f
        ...
        if (
            ...
            if match := sotn_utils.RE_PATTERNS.psp_ovl_header_entity_table_pattern.search(file_text):
                if ovl_load_address := first_address_pattern.search(file_text):
                    ovl_load_symbol = Symbol(f"{ovl_name.upper()}_Load", int(ovl_load_address.group(1), 16), None)
    ...
    template = Template(
        (Path(__file__).parent / "symexport.txt.mako").read_text()
    )
    ...
    return ovl_load_symbol, ovl_header_symbol, {"name": entity_updates_name}, symexport_text
...
def create_header_c(header_items, ovl_name, ovl_type, version, header_path):
    ...
    template = Template(
        (Path(__file__).parent / "header.c.mako").read_text()
    )
...
def create_e_init_c(entity_updates, e_inits, ovl_name, e_init_c_path):
    if entity_updates:
        ...
        template = Template(
            (Path(__file__).parent / "e_init.c.mako").read_text()
        )
...
def extract(args, version):
    ...
    with sotn_utils.Spinner(message="gathering initial symbols") as spinner:
        ...
        ovl_config.subsegments, parsed_symbols, ovl_header, entity_updates, e_inits = sotn_utils.add_initial_symbols(
            ovl_config,
            e_init_c_path,
            ovl_header_symbol,
    ...
    with sotn_utils.Spinner(
        ...
        ovl_config.subsegments = sotn_utils.find_segments(ovl_config, file_header, known_starts)
        ovl_config.write_config()
