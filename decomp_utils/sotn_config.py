...
def find_segments(ovl_config):
    ...
    known_files = [
        Box(name = "e_red_door", start = "EntityIsNearPlayer", end = f"{ovl_config.name.upper}_EntityRedDoor", default_box=True),
        Box(name = "st_update", start = "Random", end = "UpdateStageEntities", default_box=True),
        Box(name = "st_collision", start = "HitDetection", end = "EntityDamageDisplay", default_box=True),
        Box(name = "create_entity", start = "CreateEntityFromLayout", end = "CreateEntityFromEntity", default_box=True),
        Box(name = "st_init", start = "GetLangAt", end = "func_psp_09254120", default_box=True),
        Box(name = "st_common", start = "DestroyEntity", end = "ReplaceBreakableWithItemDrop", default_box=True),
        Box(name = "blit_char", start = f'{"func_psp_0923C2F8" if ovl_config.version == "pspeu" else "BlitChar"}', end = "BlitChar", default_box=True),
        Box(name = "e_misc", start = "CheckColliderOffsets", end = "PlaySfxPositional", default_box=True),
        Box(name = "e_misc_2", start = "EntityHeartDrop", end = "EntityMessageBox", default_box=True),
        Box(name = "e_stage_name", start = f'{"func_psp_0923C0C0" if ovl_config.version == "pspeu" else "StageNamePopupHelper"}', end = "EntityStageNamePopup", default_box=True),
        Box(name = "e_particles", start = f'{"func_psp_0923AD68" if ovl_config.version == "pspeu" else "EntitySoulStealOrb"}', end = "EntityEnemyBlood", default_box=True),
        Box(name = "e_collect", start = "PrizeDropFall", end = "EntityRelicOrb", default_box=True),
        Box(name = "e_room_fg", start = "EntityRoomForeground", end = "EntityRoomForeground", default_box=True),
        Box(name = "e_popup", start = "BottomCornerText", end = "BottomCornerText", default_box=True),
        Box(name = "prim_helpers", start = "UnkPrimHelper", end = "PrimDecreaseBrightness", default_box=True),
        Box(name = "e_axe_knight", start = "AxeKnightUnkFunc1", end = "EntityAxeKnightThrowingAxe", default_box=True),
        Box(name = "e_skeleton", start = "SkeletonAttackCheck", end = "UnusedSkeletonEntity", default_box=True),
        Box(name = "e_fire_warg", start = "func_801CC5A4", end = "EntityFireWargDeathBeams", default_box=True),
        Box(name = "e_warg", start = "func_801CF438", end = "EntityWargExplosionPuffTransparent", default_box=True),
        Box(name = "st_debug", start = f"{ovl_config.name.upper()}_EntityBackgroundBlock", end = f"{ovl_config.name.upper()}_EntityLockCamera", default_box=True),
        Box(name = "e_venus_weed", start = "SetupPrimsForEntitySpriteParts", end = "EntityVenusWeedSpike", default_box=True),
        Box(name = "water_effects", start = "func_801C4144", end = "EntityWaterDrop", default_box=True),
        Box(name = "e_breakable", start = "EntityBreakable", end = "EntityBreakable", default_box=True),
        Box(name = "e_jewel_sword_puzzle", start = "EntityMermanRockLeftSide", end = "EntityFallingRock2", default_box=True),
        Box(name = "e_castle_door", start = "EntityCastleDoor", end = "EntityCastleDoor", default_box=True),
        Box(name = "e_background_bushes_trees", start = "EntityBackgroundBushes", end = "EntityBackgroundTrees", default_box=True),
        Box(name = "e_sky_entities", start = "EntityLightningThunder", end = "EntityLightningCloud", default_box=True),
        Box(name = "e_trapdoor", start = "EntityTrapDoor", end = "EntityTrapDoor", default_box=True),
        Box(name = "entrance_weights", start = "UpdateWeightChains", end = "EntityPathBlockTallWeight", default_box=True),
        Box(name = "e_heartroom", start = "EntityHeartRoomSwitch", end = "EntityHeartRoomGoldDoor", default_box=True),
        Box(name = "e_cavern_door", start = "DoorCascadePhysics", end = "EntityCavernDoor", default_box=True),
        Box(name = "e_stairway", start = "EntityStairwayPiece", end = "EntityFallingRock", default_box=True),
    ]
    ...
    for match in include_asm_pattern.finditer(src_text):
        ...
        if current_function in known_starts:
            if segment_meta:
                ...
                logger.debug(f"Found text segment for {segment_meta.name} at 0x{segment_meta.offset.str}")
            ...
            segment_meta = Box(start = current_function, end = None, asm_dir = asm_dir, default_box=True)
        ...
        if segment_meta and not segment_meta.offset:
            if offset := get_symbol_offset(ovl_config, current_function):
                ...
                asm_path = Path("asm") / ovl_config.version / asm_dir / f"{current_function}.s"
                asm_text = asm_path.read_text()
                if (first_offset := re.search(rf"glabel {current_function}\s+/\*\s([0-9A-F]{{1,5}})\s", asm_text)):
        ...
        if not segment_meta.name and segment_meta.offset:
            segment_meta.name = f"{ovl_config.segment_prefix}unk_{segment_meta.offset.str}"
    ...
    file_header = get_default("file_header").format(ovl_header_path = f"../{ovl_config.name}/{ovl_config.name}.h" if ovl_config.platform == "psp" else f"{ovl_config.name}.h")
    for segment in segments:
        ...
        segment_text = src_text[segment_start:segment_end].replace(f"{ovl_config.nonmatchings_path}/{ovl_config.segment_prefix}{ovl_config.first_src_file.stem}", f"{ovl_config.nonmatchings_path}/{segment.name}").rstrip("\n")
        ...
        for rodata_symbol in include_rodata_pattern.findall(segment_text):
            ...
            rodata_subsegments.append(Box(offset = rodata_offset, type = ".rodata", name = segment.name))

        # Extract rodata offsets from assembly files referenced in INCLUDE_ASM macros
        asm_files = [ovl_config.asm_path.joinpath(ovl_config.nonmatchings_path, ovl_config.segment_prefix, ovl_config.first_src_file.stem, match.group(2)).with_suffix(".s") for match in include_asm_pattern.finditer(segment_text)]
        ...
        for asm_file in asm_files:
            ...
            if rodata_start != -1 and text_start > rodata_start:
                rodata_text = asm_text[rodata_start:text_start] if text_start > rodata_start else asm_text[rodata_start:]
                for rodata_offset in rodata_pattern.findall(rodata_text):
                    rodata_subsegments.append(Box(offset = int(rodata_offset, 16), type = ".rodata", name = segment.name))

        ovl_config.src_path.joinpath(segment.name).with_suffix(".c").write_text(file_header + segment_text + "\n")
    
    rodata_by_segment = {}
    for rodata_subsegment in rodata_subsegments:
        if rodata_subsegment.name not in rodata_by_segment or rodata_subsegment.offset < rodata_by_segment[rodata_subsegment.name][0]:
            rodata_by_segment[rodata_subsegment.name] = [rodata_subsegment.offset, rodata_subsegment.type, rodata_subsegment.name]
...
def find_symbols(parsed, version, ovl_name, threshold = 0.95):
    ...
    check_ops_by_op_hash = {k:v[0].ops.parsed for k,v in check_funcs_by_op_hash.items()}

    buckets = get_buckets((ref_ops_by_op_hash, check_ops_by_op_hash), num_buckets=20, tolerance=0.1)

    args = (
        {"ref_ops_by_op_hash": bucket[0], "check_ops_by_op_hash": bucket[1], "threshold": threshold}
    ...
    for ref_op_hash, results in group_results(results).items():
        ...
        if ref_paths and check_paths:
            ref_names = tuple(re.sub(r"^[A-Z0-9]{3,4}_(\w+)", rf"{ovl_name.upper()}_\1", func.stem) for func in ref_paths)
            check_names = tuple(func.stem for func in check_paths)
            matches.add((ref_paths, ref_names, check_paths, check_names))
    matches = tuple(Box({"ref": {"paths": ref_paths, "names": {"all": tuple(set(ref_names)), "no_defaults": tuple({name for name in ref_names if not name.startswith(f"func_{version}")})}, "counts": {"all": Counter(ref_names).most_common(), "no_defaults": Counter(tuple(name for name in ref_names if not name.startswith(f"func_{version}"))).most_common()}}, "check": {"paths": check_paths, "names": check_names}, "score": score}) for ref_paths, ref_names, check_paths, check_names in matches)
    return matches
...
def rename_symbols(ovl_config, matches):
    ...
    known_pairs = (
            ...
            Box(first = "CreateEntityWhenInVerticalRange", last = "CreateEntityWhenInHorizontalRange"),
    ...
    for match in matches:
        for pair in known_pairs:
            if len(match.ref.names.no_defaults) == 2 and len(match.check.names) <= 2 and pair.first in match.ref.names.no_defaults and pair.last in match.ref.names.no_defaults:
                offset = min(tuple(int(x.split("_")[-1], 16) for x in match.check.names))
                symbols[pair.first].append(Symbol(pair.first, offset))
                if len(match.check.names) == 2:
                    offset = max(tuple(int(x.split("_")[-1], 16) for x in match.check.names))
                    symbols[pair.last].append(Symbol(pair.last, offset))
                break
        else:
            if len(match.check.names) == 1 and match.check.names[0].startswith(f"func_{ovl_config.version}_"):
                if len(match.ref.counts.no_defaults) > 1 and match.ref.counts.no_defaults[0][1] == match.ref.counts.no_defaults[1][1]:
                    logger.warning(f"Ambiguous match for {match.ref.counts.no_defaults}")
                offset = int(match.check.names[0].split("_")[-1], 16)
                if match.ref.names.no_defaults:
                    symbols[match.ref.counts.no_defaults[0][0]].append(Symbol(match.ref.counts.no_defaults[0][0], offset))
            # This is only a stopgap until MagicallySealedDoorIsNearPlayer is decompiled for psp
            elif ovl_config.version == "pspeu" and len(match.ref.names.no_defaults) == 1 and len(match.check.names) == 2 and "EntityIsNearPlayer" in match.ref.names.no_defaults:
                offset = min(tuple(int(x.split("_")[-1], 16) for x in match.check.names))
                symbols["EntityIsNearPlayer"].append(Symbol("EntityIsNearPlayer", offset))
                offset = max(tuple(int(x.split("_")[-1], 16) for x in match.check.names))
                symbols["MagicallySealedDoorIsNearPlayer"].append(Symbol("MagicallySealedDoorIsNearPlayer", offset))
            elif match.ref.names.no_defaults != match.check.names:
                # Todo: Convert to proper log entry format
                logger.warning(json.dumps({"ref_funcs": match.ref.counts.no_defaults, "check_funcs": match.check.names, "score": match.score}))
    ...
    add_symbols(ovl_config, tuple(sorted(syms, key=lambda x: x.address, reverse=True)[0] for syms in symbols.values()))
    return len(symbols)
...
def parse_psp_stage_init(asm_path):
    ...
    for file in asm_path.rglob("*.s"):
        ...
        if " 06A8240E " in file_text and " 1D09043C " in file_text and " 38F78424 " in file_text and " E127240E " in file_text and " C708023C " in file_text and " 30BC43AC " in file_text:
            match = re.search(r"""
        ...
        # I'm pretty sure all of these are found in the same file, but keeping this here just in case
            ...
            if " 1D09043C " in file_text and " 38F78424 " in file_text and " E127240E " in file_text:
                match = re.search(r"""
                    \s+/\*\s[A-F0-9]{1,5}\s[A-F0-9]{8}\s1D09043C\s\*/.*\n
                    \s+/\*\s[A-F0-9]{1,5}\s[A-F0-9]{8}\s38F78424\s\*/.*\n
                    \s+/\*\s[A-F0-9]{1,5}(?:\s[A-F0-9]{8}){2}\s\*/\s+lui\s+\$a1,\s+%hi\((?P<export>[A-Za-z0-9_]+)\)\n
...
def parse_psx_header(ovl_name, data_file_text):
    ...
    matches = re.findall(r"/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s([0-9A-F]{8})\s\*/\s+\.word\s+\w+", header)
    if matches:
        if len(matches) > 7:
            pStObjLayoutHorizontal_address = int.from_bytes(bytes.fromhex(matches[7]), "little")
        else:
...
def parse_init_room_entities(ovl_name, platform, init_room_entities_path):
    symbol_pattern = re.compile(r"\s+/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s[0-9A-F]{8}\s\*/\s+[a-z]{1,5}[ \t]*\$\w+,\s%hi\(D_(?:\w+_)?([A-F0-9]{8})\)\s*")
    ...
    symbols = tuple(Symbol(name, int(symbol_pattern.fullmatch(lines[i]).group(1),16)) for name, i in init_room_entities_map.items() if "(D_" in lines[i])
    create_entity_bss_address = min(x.address for x in symbols if x.name.startswith("g_Layout"))
...
def parse_export_table(ovl_type, export_table_symbol, data_file_text):
    ...
    if export_table_start != -1:
        matches = re.finditer(r"/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s(?P<address>[0-9A-F]{8})\s\*/\s+\.word\s+func_\w+", data_file_text[export_table_start:export_table_end])

    if matches:
        return tuple(
            Symbol(name, int.from_bytes(bytes.fromhex(match.group("address")), "little"))
...
def parse_entity_table(ovl_name, entity_table_symbol, data_file_text):
    ...
    if entity_table_start != -1:
        ...
        matches = re.findall(r"/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s([0-9A-F]{8})\s\*/\s+\.word\s+func_\w+", parsed_entity_table)
