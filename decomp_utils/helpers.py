...
RE_TEMPLATES = namedtuple("ReTemplates", ["sym_replace", "find_symbol_by_name", "rodata_offset", "asm_symbol_offset"])(
    sym_replace = Template(r"(?:D_|func_)${sym_prefix}(${symbols_list})"),
    find_symbol_by_name = Template(r"\n\s+0x(?P<address>[A-Fa-f0-9]{8})\s+${symbol_name}\n"),
    rodata_offset = Template(r"glabel (?:jtbl|D)_${version}_[0-9A-F]{8}\n\s+/\*\s(?P<offset>[0-9A-F]{1,5})\s"),
    asm_symbol_offset = Template(r"glabel ${symbol_name}\s+/\*\s(?P<offset>[0-9A-F]{1,5})\s"),
)
RE_PATTERNS = namedtuple("RePatterns", [
    "symbol_file_line",
    "op",
    "asm_line",
    "jtbl",
    ...
    map_symbol = re.compile(r"\n\s+0x(?P<address>[A-Fa-f0-9]{8})\s+(?P<name>[A-Za-z]\w+)\n"),
    elf_symbol = re.compile(r"(?P<address>[A-Fa-f0-9]{8})\s+[^A]\s+(?P<name>[A-Za-z]\w+)"),
    include_asm = re.compile(r'INCLUDE_ASM\("(?P<dir>[A-Za-z0-9/_]+)",\s?(?P<name>\w+)\);'),
    include_rodata = re.compile(r'INCLUDE_RODATA\("[A-Za-z0-9/_]+",\s?(?P<name>\w+)\);'),
    camel_case = re.compile(r"([A-Za-z])([A-Z][a-z])"),
    symbol_ovl_name_prefix = re.compile(r"^[^U][A-Z0-9]{3,4}"),
    ...
    psp_entity_export_table_pattern = re.compile(rf"{RE_STRINGS.psp_entity_table}(?:.*\n)+{RE_STRINGS.psp_export_table}", re.VERBOSE),
    psx_header_line_pattern = re.compile(r"/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s(?P<address>[0-9A-F]{8})\s\*/\s+\.word\s+(?P<name>\w+)"),
    init_room_entities_symbol_pattern = re.compile(r"\s+/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s[0-9A-F]{8}\s\*/\s+[a-z]{1,5}[ \t]*\$\w+,\s%hi\(D_(?:\w+_)?(?P<address>[A-F0-9]{8})\)\s*"),
    symbol_table_line = re.compile(r"/\*\s[0-9A-F]{1,5}\s[0-9A-F]{8}\s(?P<address>[0-9A-F]{8})\s\*/\s+\.word\s+func_\w+"),
    ...
    cross_ref_address_pattern = re.compile(r"lui\s+.+?%hi\((?:D_|func_)(?:\w+_)?([A-F0-9]{8})\)"),
