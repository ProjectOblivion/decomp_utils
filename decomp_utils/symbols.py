...
def remove_orphans_from_config(config_path):
    ...
    symbol_file = Path(symbol_addrs_path) if isinstance(symbol_addrs_path, str) else Path(symbol_addrs_path[-1])
    symbol_lines = re.finditer(r"(?P<name>\w+)\s*=\s*0x[A-Fa-f0-9]{8};.*\n", symbol_file.read_text())

    symbols = {match.group("name"):match.group(0).rstrip("\n") for match in symbol_lines}
    asm_path = Path(config["options"]["asm_path"])
    asm_file_list = list(asm_path.rglob("*.s"))
    if not asm_file_list:
        logger.error(f"No files found in '{asm_path}' to extract symbols from for '{symbol_file}', making no changes.")
        raise SystemExit
    ...
    for asm_file in asm_file_list:
        ...
        for symbol in remaining_symbols:
            ...
            if symbol_index != -1:
                symbol_slice = content[symbol_index-1:symbol_index+len(symbol)+1]
                if "ignore:true" in symbols[symbol] or "allow-duplicates:true" in symbols[symbol] or re.search(rf"\b{symbol}\b", symbol_slice):
...
def add_symbols(ovl_config, add_symbols):
    ...
    existing_symbols = re.finditer(r"(?P<name>\w+)\s=\s0x(?P<address>[A-Fa-f0-9]{8})", symbols_text)
    existing_symbols = {symbol.group("address"): symbol.group("name") for symbol in existing_symbols}
    # Any addresses not in the ovl vram address space are global and should not be included in the ovl symbols file
    new_symbols = {f"{symbol.address:08X}":symbol.name for symbol in add_symbols if symbol.address >= ovl_config.vram and symbol.name not in symbols_text and (f"{symbol.address:08X}" not in existing_symbols or (existing_symbols[f"{symbol.address:08X}"].startswith("D_") and existing_symbols[f"{symbol.address:08X}"].startswith("func_")))}

    if new_symbols:
        new_lines = [f"{name} = 0x{address};" for address, name in sorted((existing_symbols | new_symbols).items())]
        ...
        for src_file in (dirpath/f for dirpath, _, filenames in ovl_config.src_path_full.walk() for f in filenames if f.endswith(".c")):
            src_text = src_file.read_text()
            adjusted_text = pattern.sub(lambda match: new_symbols[match.group(1)], src_text)
        ...
        if ovl_config.symexport_path and ovl_config.symexport_path.exists():
            adjusted_text = pattern.sub(lambda match: new_symbols[match.group(1)], ovl_config.symexport_path.read_text())
...
def force_symbols(version, elf_files):
    ...
    for elf_file in (x for x in elf_files if version != "pspeu" or "dra" not in x.name):
        config = yaml.safe_load(Path(f"config/splat.{version}.{elf_file.stem}.yaml").open())
        symbols_path = next((Path(path) for path in config["options"]["symbol_addrs_path"] if elf_file.stem in path),None)
        if symbols_path.exists():
            excluded_starts = {"LM", "__pad"}
            excluded_ends = {"_START", "_END", "_VRAM"}
            symbols_lines = [f"{symbol.name} = 0x{symbol.address:08X}; // allow_duplicated:True" for symbol in get_symbols(elf_file, excluded_starts, excluded_ends)]
            symbols_path.write_text(f"{"\n".join(symbols_lines)}\n")
        else:
            logger.warning(f"No symbols file found for {elf_file}, skipping")

def get_symbols(file_path, excluded_starts = ["LM", "__pad"], excluded_ends = ["_END", "_START", "_VRAM"], no_default = False):
    symbols = set()
    match file_path.suffix:
        ...
        case ".elf":
            ...
            matches = re.finditer(r"(?P<address>[A-Fa-f0-9]{8})\s+[^A]\s+(?P<name>[A-Za-z]\w+)", text)
        case _:
            raise SystemError("File to extract symbols from must be either .elf or .map")

    for match in matches:
        symbol_name = match.group("name")
        if not no_default or (not symbol_name.startswith("func_") and not symbol_name.startswith("D_")):
            filters = "_compiled" not in symbol_name and not symbol_name.endswith("_c")
            if filters and not any(symbol_name.startswith(x) for x in excluded_starts) and not any(symbol_name.endswith(x) for x in excluded_ends):
