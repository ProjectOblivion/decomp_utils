...
def main(args):
    ...
    if ovl_config.config_path.exists() and not args.force:
        logger.error(f"A configuration for {ovl_config.name} already exists.  Use the -f/--force option to remove all existing overlay artifacts and recreate the configuration.")
        raise SystemExit
    else:
        with decomp_utils.Spinner(message = "ensuring no overlay artifacts exist"):
            ...
            if ovl_config.ovl_symbol_addrs_path.exists():
                decomp_utils.shell(f"git rm --cached -f {ovl_config.ovl_symbol_addrs_path}")
            ...
            ovl_config.build_path.joinpath(f"{ovl_config.target_path.name}").unlink(missing_ok=True)
    ...
    with decomp_utils.Spinner(message = "creating initial files") as spinner:
        ...
        header_path = ovl_config.src_path_full.with_name(ovl_config.name)/f"{ovl_config.name}.h"
        ovl_header_text = decomp_utils.get_default("ovl.h").format(ovl_name = ovl_config.name.upper())
    ...
    with decomp_utils.Spinner(message = f"gathering initial symbols") as spinner:
        ...
        first_data_index = next(i for i,subseg in enumerate(ovl_config.subsegments) if "data" in subseg)
        first_data_path = ovl_config.asm_path/"data"/f"{ovl_config.subsegments[first_data_index][0]:X}.data.s"
        first_data_text = first_data_path.read_text() if first_data_path.exists() else None
        if ovl_config.platform == "psx" and first_data_text:
            spinner.message = f"parsing the psx header for symbols"
            pStObjLayoutHorizontal_address, header_symbols = decomp_utils.parse_psx_header(ovl_config.name, first_data_text)

        if ovl_config.platform == "psp":
            spinner.message = f"parsing the psp stage init for symbols"
            stage_init, export_table_symbol, entity_table_symbol = decomp_utils.parse_psp_stage_init(ovl_config.asm_path.joinpath(ovl_config.nonmatchings_path))
            ...
            pStObjLayoutHorizontal_index = first_data_text.find(f"{pStObjLayoutHorizontal_address:08X}", end_of_header)
            first_entity_index = first_data_text.find(" func", pStObjLayoutHorizontal_index)
            entity_table_index = first_data_text.rfind("glabel", pStObjLayoutHorizontal_index, first_entity_index)
            entity_table_symbol = first_data_text[entity_table_index:first_entity_index].split("\n")[0].split()[1]

        if entity_table_symbol:
            spinner.message = f"parsing the entity table for symbols"
            entity_table_address, entity_table_symbols = decomp_utils.parse_entity_table(ovl_config.name, entity_table_symbol, first_data_text)

        if export_table_symbol:
            spinner.message = f"parsing the export table for symbols"
            export_table_symbols = decomp_utils.parse_export_table(ovl_config.ovl_type, export_table_symbol, first_data_text)

        if header_symbols or entity_table_symbols or export_table_symbols:
            spinner.message = f"adding parsed symbols"
            parsed_symbols = tuple(
                symbol
                for symbols in (header_symbols, entity_table_symbols, export_table_symbols)
                if symbols is not None
                for symbol in symbols
            ) + (decomp_utils.Symbol(f"{ovl_config.name.upper()}_EntityUpdates", entity_table_address),)
            decomp_utils.add_symbols(ovl_config, parsed_symbols)
            decomp_utils.shell(f"git clean -fdx {ovl_config.asm_path}")
            spinner.message = "splitting using updated symbols"
            decomp_utils.splat_split(ovl_config.config_path)

    with decomp_utils.Spinner(message = "extracting reference symbols from .elf files") as spinner:
        if (elf_files := ovl_config.build_path.glob("*.elf")):
            decomp_utils.force_symbols(ovl_config.version, elf_files)
        else:
            logger.error(f"No elf files found in {ovl_config.build_path}.  Rerun this tool after a successful build.")
            raise SystemExit

    with decomp_utils.Spinner(message = "disassembling reference functions"):
        decomp_utils.shell(f"git clean -fdx asm/{ovl_config.version}/ -e {ovl_config.asm_path}")
        ref_configs, ref_ovls = [], []
        for file in Path("config").iterdir():
            ref_version = "pspeu" in file.name if ovl_config.platform == "psp" else "us" in file.name or "hd" in file.name
            exclude = not file.match("splat.*.yaml") or ovl_config.name in file.name or "main" in file.name or "weapon" in file.name or "mad" in file.name 
            # Todo: This should probably differentiate between stage and non-stage to reduce execution time
            if ref_version and not exclude and (match := re.match(rf"splat\.\w+\.(?:st|bo)?(?P<ref_ovl>\w+)\.yaml", file.name)):
    ...
    with decomp_utils.Spinner(message = f"parsing instructions from reference overlay asm files") as spinner:
        ...
        for dirpath, _, filenames in Path("asm").joinpath(ovl_config.version).walk():
            if any(x in dirpath.parts or f"{x}_psp" in dirpath.parts for x in ref_ovls):
                ref_files.extend(dirpath/f for f in filenames if not f.startswith(f"func_{ovl_config.version}_") and not f.startswith("D_"))
            if ovl_config.name in dirpath.parts or f"{ovl_config.name}_psp" in dirpath.parts:
                check_files.extend(dirpath/f for f in filenames if f.startswith(f"func_{ovl_config.version}_"))
        parsed = Box(ref_files = decomp_utils.parse_files(ref_files), check_files = decomp_utils.parse_files(check_files))

    with decomp_utils.Spinner(message = "finding symbol names using reference overlays") as spinner:
        matches = decomp_utils.find_symbols(parsed, ovl_config.version, ovl_config.name, threshold = 0.95)
        num_symbols = decomp_utils.rename_symbols(ovl_config, matches)

    if num_symbols:
        with decomp_utils.Spinner(message = f"renamed {num_symbols} symbols, splitting again"):
            decomp_utils.shell(f"git clean -fdx {ovl_config.asm_path}")
            decomp_utils.splat_split(ovl_config.config_path, ovl_config.disassemble_all)

    nonmatchings_path = f"{ovl_config.nonmatchings_path}/{ovl_config.name}_psp" if ovl_config.platform == "psp" else ovl_config.nonmatchings_path
    init_room_entities_path = ovl_config.asm_path/nonmatchings_path/f"first_{ovl_config.name}"/f"InitRoomEntities.s"
    if init_room_entities_path.exists():
        with decomp_utils.Spinner(message = f"parsing InitRoomEntities.s for symbols"):
            init_room_entities_symbols, create_entity_bss_address = decomp_utils.parse_init_room_entities(ovl_config.name, ovl_config.platform, init_room_entities_path)
    
            # Todo: Add bss segment comment
            if create_entity_bss_address:
                create_entity_bss_start = create_entity_bss_address - ovl_config.vram + ovl_config.start
                create_entity_bss_end = create_entity_bss_start + (0x18 if ovl_config.platform == "psp" else 0x10)

            if init_room_entities_symbols:
                decomp_utils.add_symbols(ovl_config, init_room_entities_symbols)

    with decomp_utils.Spinner(message = "looking for functions for cross referencing") as spinner:
        parsed.check_files = decomp_utils.parse_files(dirpath/f for dirpath, _, filenames in ovl_config.asm_path.walk() for f in filenames if not f.startswith(f"func_{ovl_config.version}_") and not f.startswith("D_"))
        ...
        for ref_file in parsed.ref_files:
            ...
            if ref_file.path.name in check_files_by_name and ref_file.instructions.normalized == check_files_by_name[ref_file.path.name].instructions.normalized:
        ...
        if ref_files_by_name:
            ...
            address_pattern = re.compile(r"lui\s+.+?%hi\((?:D_|func_)(?:\w+_)?([A-F0-9]{8})\)")
            ...
            for file_name, parsed_ref_files in ref_files_by_name.items():
                ...
                for parsed_ref_file in parsed_ref_files:
                    for ref_instruction, check_instruction in zip(parsed_ref_file.instructions.parsed, parsed_check_file.instructions.parsed):
                        # Todo: Warn if a symbol is not default and does not match the cross referenced symbol
                        name, address = name_pattern.match(ref_instruction), address_pattern.match(check_instruction)
                        if name and not name.group(1).startswith("D_") and not name.group(1).startswith(f"func_{ovl_config.version}_") and address:
                            new_syms.add(decomp_utils.Symbol(re.sub(r"^[A-Z0-9]{3,4}_(\w+)", rf"{ovl_config.name.upper()}_\1", name.group(1)), int(address.group(1), 16)))

    if new_syms:
        with decomp_utils.Spinner(message = f"adding {len(new_syms)} cross referenced symbols and splitting again"):
    ...
    with decomp_utils.Spinner(message = f"creating {ovl_config.ld_script_path.with_suffix(".elf")}"):
        decomp_utils.build([f'{ovl_config.ld_script_path.with_suffix(".elf")}'], version=ovl_config.version)

    with decomp_utils.Spinner(message = f"finding segments and splitting source files") as spinner:
        first_text_index = next(i for i,subseg in enumerate(ovl_config.subsegments) if "c" in subseg)
        segments, rodata_segments = decomp_utils.find_segments(ovl_config)
        text_subsegs = [decomp_utils.yaml.FlowSegment([segment.offset.int, "c", segment.name]) for segment in segments]
        ...
        first_rodata_index = next(i for i,subseg in enumerate(ovl_config.subsegments) if ".rodata" in subseg)
        ovl_config.subsegments[first_rodata_index:first_rodata_index+1] = rodata_subsegs

    ovl_config.write_config()

    with decomp_utils.Spinner(message = f"extracting overlay to validate configuration") as spinner:
        # Todo: Compare generated offsets to .elf segment offsets
        decomp_utils.shell(f"git clean -fdx {ovl_config.asm_path}")
        ovl_config.ld_script_path.unlink(missing_ok=True)
        output = decomp_utils.build([f"{ovl_config.ld_script_path}"], version=ovl_config.version).decode()
        ...
        for match in splat_suggestions:
            suggestions = re.findall(r"\s+-\s+\[(0x[0-9A-Fa-f]+),\s(.+?)\]", match.group("suggestions"))
            suggested_segments.extend([offset, segment_type, match.group("segment")] for offset, segment_type in suggestions)
    ...
    with decomp_utils.Spinner(message = f"building and validating {built_bin}"):
        decomp_utils.build([f'{ovl_config.ld_script_path.with_suffix(".elf")}', ovl_config.name], concurrent=True, version=ovl_config.version)
        if built_bin.exists():
        ...
        if ovl_config.sha1 != built_sha1:
            ...
            lines = check_file_path.read_text().splitlines()
            bin_line = f"{ovl_config.sha1}  build/{ovl_config.version}/{ovl_config.target_path.name}"
            if f"#{bin_line}" in lines:
                bin_index = lines.index(f"#{bin_line}")
                lines[bin_index] = bin_line
            fbin_path = ovl_config.target_path.with_name(f"{"f" if ovl_config.version == "pspeu" else "F"}_{ovl_config.target_path.name}")
            if fbin_path.exists():
                fbin_sha1 = hashlib.sha1(fbin_path.read_bytes()).hexdigest()
                fbin_line = f"{fbin_sha1}  build/{ovl_config.version}/{fbin_path.name}"
                if f"#{fbin_line}" in lines:
                    fbin_index = lines.index(f"#{fbin_line}")
                    lines[fbin_index] = fbin_line
            # Todo: Order the sha1 lines correctly
            check_file_path.write_text(f"{"\n".join(lines)}\n")
            decomp_utils.shell(f"git add {ovl_config.config_path} {ovl_config.ovl_symbol_addrs_path} {check_file_path}")
